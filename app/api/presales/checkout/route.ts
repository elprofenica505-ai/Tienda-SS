...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json(); const presaleId = text(body.presaleId, 120); const paymentMethod = ['cash', 'card', 'transfer', 'credit'].includes(body.paymentMethod) ? body.paymentMethod : ''; const customerId = text(body.customerId, 120); const branchId = text(body.branchId, 120) || request.headers.get('x-branch-id')?.trim() || context.branchIds[0] || ''; const warehouseId = text(body.warehouseId, 128) || 'warehouse-main';
    ...
    if (!branchId) return NextResponse.json({ error: 'Selecciona una sucursal antes de cobrar el ticket.' }, { status: 400 });
    ...
    if (paymentMethod !== 'credit' && !openSession) return NextResponse.json({ error: 'Abre una sesión de caja antes de cobrar tickets.' }, { status: 409 });
    const db = getAdminDb(); const tenant = db.collection('tenants').doc(context.tenantId); const presaleRef = tenant.collection('presales').doc(presaleId); const saleRef = tenant.collection('sales').doc(); const statsRef = tenant.collection('stats').doc('daily').collection('days').doc(new Date().toISOString().slice(0, 10)); const fiscalRef = tenant.collection('settings').doc('fiscal'); const cashSessionRef = openSession ? tenant.collection('cashSessions').doc(openSession.id) : null;
    const result = await db.runTransaction(async (transaction) => {
      ...
      const productRefs = Array.from(itemMap.keys()).map((id) => tenant.collection('products').doc(id)); const stockRefs = Array.from(itemMap.keys()).map((id) => tenant.collection('inventoryStocks').doc(stockKey(warehouseId, id))); const movementRefs = productRefs.map(() => tenant.collection('inventoryMovements').doc()); const customerRef = customerId ? tenant.collection('customers').doc(customerId) : null;
      const tenantSnapshot = await transaction.get(tenant); const fiscalSnapshot = await transaction.get(fiscalRef); const cashSessionSnapshot = cashSessionRef ? await transaction.get(cashSessionRef) : null; if (cashSessionRef && (!cashSessionSnapshot?.exists || cashSessionSnapshot.data()?.status !== 'open')) throw new Error('CASH_SESSION_NOT_OPEN'); const productSnapshots = await transaction.getAll(...productRefs); const stockSnapshots = await transaction.getAll(...stockRefs); const customerSnapshot = customerRef ? await transaction.get(customerRef) : null;
      ...
      productSnapshots.forEach((snapshot, index) => { if (!snapshot.exists || snapshot.data()?.active === false) throw new Error('PRODUCT_NOT_FOUND'); const data = snapshot.data() || {}; const stockSnapshot = stockSnapshots[index]; const stock = stockSnapshot?.exists ? inventoryNumber(stockSnapshot.data()?.quantity) : warehouseId === 'warehouse-main' ? inventoryNumber(data.stock) : 0; const quantity = itemMap.get(productRefs[index].id) || 0; if (data.itemType !== 'service' && stock < quantity) throw new Error(`INSUFFICIENT_STOCK:${data.name || productRefs[index].id}`); });
      const subtotal = lines.reduce((sum, line) => sum + line.total, 0); const fiscal = createFiscalSaleFields({ ...body, customerName }, subtotal, 0); const fiscalError = validateFiscalFields(fiscal); if (fiscalError) throw new Error(`FISCAL_INVALID:${fiscalError}`); const fiscalConfig = normalizeFiscalConfig(fiscalSnapshot.exists ? fiscalSnapshot.data() || {} : {}); const sequence = fiscalConfig.nextInvoiceSequence; const invoiceNumber = formatFiscalNumber(typeof fiscalConfig.invoicePrefix === 'string' ? fiscalConfig.invoicePrefix : 'FAC', sequence); const total = fiscalMoney(fiscal.total); const customerCreditBalance = customerSnapshot ? money(customerSnapshot.data()?.creditBalance) : 0; const customerCreditLimit = customerSnapshot ? money(customerSnapshot.data()?.creditLimit) : 0; const creditOverride = body.creditOverride === true && ['owner', 'admin'].includes(context.role); if (paymentMethod === 'credit' && customerCreditBalance + total > customerCreditLimit && !creditOverride) throw new Error(`CREDIT_LIMIT_EXCEEDED:${customerCreditLimit}:${customerCreditBalance}`); const now = new Date();
      productSnapshots.forEach((snapshot, index) => { const data = snapshot.data() || {}; if (data.itemType === 'service') return; const quantity = itemMap.get(productRefs[index].id) || 0; const stockSnapshot = stockSnapshots[index]; const previousStock = stockSnapshot?.exists ? inventoryNumber(stockSnapshot.data()?.quantity) : warehouseId === 'warehouse-main' ? inventoryNumber(data.stock) : 0; const newStock = stockAfterDelta(previousStock, -quantity); transaction.set(stockRefs[index], { warehouseId, branchId, productId: productRefs[index].id, quantity: newStock, averageCost: inventoryNumber(stockSnapshot?.data()?.averageCost || data.averageCost), updatedAt: now, updatedBy: context.uid }, { merge: true }); if (warehouseId === 'warehouse-main') transaction.update(productRefs[index], { stock: newStock, updatedAt: now, updatedBy: context.uid }); transaction.set(movementRefs[index], { warehouseId, branchId, productId: productRefs[index].id, type: 'sale', quantity, delta: -quantity, previousStock, newStock, reason: `Preventa ${presale.ticketCode || presaleId}`, saleId: saleRef.id, presaleId, createdBy: context.uid, createdAt: now }); });
      const paidAmount = paymentMethod === 'credit' ? 0 : total; const cashReceived = paymentMethod === 'cash' ? Math.max(total, money(body.cashReceived || total)) : paidAmount; const changeAmount = paymentMethod === 'cash' ? Math.max(0, cashReceived - total) : 0;
      transaction.set(saleRef, { saleNumber: `V-${Date.now().toString(36).toUpperCase()}`, invoiceNumber, presaleId, ticketCode: presale.ticketCode, items: lines, subtotal, discount: 0, taxableBase: fiscal.taxableBase, exemptAmount: fiscal.exemptAmount, taxRate: fiscal.taxRate, taxAmount: fiscal.taxAmount, total, paidAmount, cashReceived, changeAmount, balanceDue: paymentMethod === 'credit' ? total : 0, paymentStatus: paymentMethod === 'credit' ? 'pending' : 'paid', dueAt: paymentMethod === 'credit' ? new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000) : null, currency: fiscal.currency, customerId: customerId || null, customerName: customerName || fiscal.customerName || null, customerRuc: fiscal.customerRuc || null, customerAddress: fiscal.customerAddress || null, paymentMethod, branchId, warehouseId, cashSessionId: openSession?.id || null, fiscal: { provider: fiscalConfig.provider, mode: fiscalConfig.mode, status: fiscalConfig.mode === 'manual' ? 'not_requested' : 'pending', adapterVersion: 'adapter-core-2026-09' }, status: 'completed', createdBy: context.uid, vendedorUid: presale.vendedorUid, createdAt: now, updatedAt: now });
    ...
    if (message === 'CASH_SESSION_NOT_OPEN') return NextResponse.json({ error: 'La sesión de caja se cerró antes de completar el cobro.' }, { status: 409 });
