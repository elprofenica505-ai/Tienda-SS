...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json(); const presaleId = text(body.presaleId, 120); const paymentMethod = ['cash', 'card', 'transfer', 'credit'].includes(body.paymentMethod) ? body.paymentMethod : ''; const customerId = text(body.customerId, 120); const branchId = text(body.branchId, 120) || request.headers.get('x-branch-id')?.trim() || context.branchIds[0] || '';
