...
async function contextFor(request: NextRequest, action: 'view' | 'create' | 'edit') { const context = await requireTenantPermission(request, 'finance', action); const branchId = text(request.headers.get('x-branch-id'), 128) || context.branchIds[0] || ''; if (!branchId) throw new Error('BRANCH_REQUIRED'); assertBranchAccess(context, branchId); return { context, branchId }; }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await getSupabaseServer().rpc('cash_session_action', { target_action: action, target_tenant_id: context.tenantId, target_branch_id: branchId, target_register_id: registerId || null, target_session_id: sessionId || null, target_user_id: context.uid, target_payload: payload });
    if (result.error) throw new Error(result.error.message);
