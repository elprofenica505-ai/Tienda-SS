...
function mapSession(row: any, stats: { movementCount: number; salesCount: number; salesTotal: number; expectedByMethod: Record<string, number> } = { movementCount: 0, salesCount: 0, salesTotal: 0, expectedByMethod: {} }) { return { id: row.id, branchId: row.branch_id, registerId: row.cash_register_id, status: row.status, openingByMethod: row.metadata?.openingByMethod || { cash: Number(row.opening_amount || 0), card: 0, transfer: 0 }, expectedByMethod: row.metadata?.expectedByMethod || (row.status === 'open' ? stats.expectedByMethod : null), countedByMethod: row.metadata?.countedByMethod || null, difference: row.metadata?.difference == null ? null : Number(row.metadata.difference), salesCount: stats.salesCount, salesTotal: stats.salesTotal, movementCount: stats.movementCount, openedBy: row.opened_by, openedAt: row.opened_at, closedBy: row.closed_by, closedAt: row.closed_at, updatedAt: row.updated_at, currency: row.metadata?.currency || 'NIO' }; }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const sessionIds = rawSessions.map((row: any) => row.id);
    const movementResult = sessionIds.length ? await getSupabaseServer().from('cash_movements').select('cash_session_id,amount,movement_type,reference_type,metadata').eq('tenant_id', context.tenantId).in('cash_session_id', sessionIds).limit(5000) : { data: [], error: null } as any;
    if (movementResult.error) throw new Error(movementResult.error.message);
    const statsBySession = new Map<string, { movementCount: number; salesCount: number; salesTotal: number; expectedByMethod: Record<string, number> }>();
    for (const row of rawSessions) { const opening = row.metadata?.openingByMethod || { cash: Number(row.opening_amount || 0), card: 0, transfer: 0 }; statsBySession.set(row.id, { movementCount: 0, salesCount: 0, salesTotal: 0, expectedByMethod: { cash: Number(opening.cash || 0), card: Number(opening.card || 0), transfer: Number(opening.transfer || 0) } }); }
    for (const movement of movementResult.data || []) { const stats = statsBySession.get(movement.cash_session_id); if (!stats) continue; const amount = Number(movement.amount || 0); stats.movementCount += 1; if (movement.reference_type === 'sale' || movement.movement_type === 'sale') { stats.salesCount += 1; stats.salesTotal += Math.max(0, amount); } const method = movement.metadata?.paymentMethod === 'card' ? 'card' : movement.metadata?.paymentMethod === 'transfer' ? 'transfer' : 'cash'; stats.expectedByMethod[method] += amount; }
    const sessions = rawSessions.map((row: any) => mapSession(row, statsBySession.get(row.id)));
