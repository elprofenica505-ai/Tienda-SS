...
export async function GET(request: NextRequest) {
  try { const context = await requireTenantMember(request); const db = getAdminDb(); const tenantRef = db.collection('tenants').doc(context.tenantId); const tenant = await tenantRef.get(); const data = tenant.data() || {}; const plan = data.plan || 'starter'; const [membersCount, productsCount] = await Promise.all([tenantRef.collection('members').where('status', '==', 'active').count().get(), tenantRef.collection('products').where('active', '==', true).count().get()]); return NextResponse.json({ ok: true, plans, usage: { members: membersCount.data().count, products: productsCount.data().count }, subscription: { plan, limits: getPlanLimits(plan), status: data.subscriptionStatus || 'inactive', currentPeriodEnd: data.subscriptionCurrentPeriodEnd || null, cancelAtPeriodEnd: Boolean(data.cancelAtPeriodEnd), hasPaymentCustomer: Boolean(data.stripeCustomerId) } }); }
