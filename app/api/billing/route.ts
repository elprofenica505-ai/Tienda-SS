...
export async function GET(request: NextRequest) {
  try { const context = await requireTenantMember(request); const tenant = await getAdminDb().collection('tenants').doc(context.tenantId).get(); const data = tenant.data() || {}; const plan = data.plan || 'starter'; return NextResponse.json({ ok: true, plans, subscription: { plan, limits: getPlanLimits(plan), status: data.subscriptionStatus || 'inactive', currentPeriodEnd: data.subscriptionCurrentPeriodEnd || null, cancelAtPeriodEnd: Boolean(data.cancelAtPeriodEnd), hasPaymentCustomer: Boolean(data.stripeCustomerId) } }); }
