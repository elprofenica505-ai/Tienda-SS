...
function MembersContent() {
  ...
  const [message, setMessage] = useState(''); const [invitationLink, setInvitationLink] = useState('');
  ...
  async function createInvitation(event: FormEvent) {
    ...
    if (!authUser || !tenant) return;
    setSaving(true); setMessage(''); setInvitationLink('');
    try {
      ...
      if (data.delivery === 'sent') setMessage(`Invitación enviada a ${form.email}. La persona creará su cuenta al aceptar.`); else { setMessage(`La invitación quedó creada, pero el correo no está configurado o falló (${data.delivery || 'sin estado'}). Comparte el enlace manualmente.`); setInvitationLink(data.invitationUrl || ''); }
  ...
  async function claimAdminAccess() {
    ...
    setSaving(true); setMessage('');
    try {
      const response = await fetch('/api/members', { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${await authUser.getIdToken()}`, 'x-tenant-id': tenant.id }, body: JSON.stringify({ uid: member.uid, selfAccess: 'admin' }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'No se pudo actualizar tu acceso.');
      setMessage(data.alreadyFullAccess ? 'Tu cuenta ya tiene acceso total.' : 'Acceso de administrador activado. Actualizando sesión...');
      window.setTimeout(() => window.location.reload(), 500);
    } catch (error) { setMessage(error instanceof Error ? error.message : 'No se pudo actualizar tu acceso.'); }
    finally { setSaving(false); }
  }
