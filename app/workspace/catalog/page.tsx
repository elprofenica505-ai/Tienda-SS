...
type Editing = { type: 'category' | 'product'; id: string; name: string; sku: string; price: string; cost: string; stock: string; categoryId: string; active: boolean; imageUrl?: string | null; imageDataUrl?: string };
...
function CatalogContent() {
  ...
  function startEditProduct(item: Product) { setEditing({ type: 'product', id: item.id, name: item.name, sku: item.sku || '', price: String(item.price || 0), cost: String((item as Product & { cost?: number }).cost || 0), stock: String(item.stock || 0), categoryId: item.categoryId || '', active: item.active, imageUrl: item.imageUrl || null, imageDataUrl: '' }); setShowForm(false); }
