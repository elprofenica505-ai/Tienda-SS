...
function OrganizationContent() {
  ...
  const { authUser, tenant, member, organization, activeBranchId, setActiveBranchId, loading: tenantLoading, refresh } = useTenant();
  ...
  async function setBranchActive(branchId: string, active: boolean) {
    ...
    setSaving(true); setMessage('');
    try {
      const response = await fetch('/api/organization', { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${await authUser.getIdToken()}`, 'x-tenant-id': tenant.id }, body: JSON.stringify({ resource: 'branches', id: branchId, active }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || 'No se pudo actualizar la sucursal.');
      setMessage(active ? 'Sucursal reactivada.' : 'Sucursal archivada. Su historial se conserva.'); await refresh({ force: true });
    } catch (error) { setMessage(error instanceof Error ? error.message : 'No se pudo actualizar la sucursal.'); }
    finally { setSaving(false); }
  }
