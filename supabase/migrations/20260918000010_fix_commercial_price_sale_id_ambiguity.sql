-- Fix: qualify PL/pgSQL result variables used by commercial POS wrappers.
-- The previous wrappers used sale_id in an INSERT where it was ambiguous with
-- the target column, producing SQLSTATE 42702 after pricing succeeded.
begin;
...
begin
  ...
  sale_result := public.create_sale(
    ...
    target_customer_id, target_user_id, target_payment_method, discount_value,
    target_idempotency_key, metadata_value, priced.items
  );
