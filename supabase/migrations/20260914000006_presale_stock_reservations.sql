-- P0: bind presales to inventory reservations and keep physical quantity separate.
alter table public.presales add column if not exists reservation_id uuid;
alter table public.presales add column if not exists warehouse_id uuid;
create index if not exists presales_reservation_idx on public.presales (tenant_id, reservation_id);
...
create or replace function public.consume_inventory_reservation(target_tenant_id uuid,target_reservation_id uuid,target_user_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
declare reservation_row public.inventory_reservations%rowtype; item jsonb; product_id uuid; consumed numeric; stock_row public.inventory_stocks%rowtype;
begin
  select * into reservation_row from public.inventory_reservations where id=target_reservation_id and tenant_id=target_tenant_id for update;
  if not found then raise exception 'RESERVATION_NOT_FOUND'; end if;
  if reservation_row.status <> 'active' then raise exception 'RESERVATION_CLOSED'; end if;
  for item in select value from jsonb_array_elements(reservation_row.items) loop
    product_id := nullif(item->>'productId', '')::uuid; consumed := (item->>'quantity')::numeric;
    if product_id is null or consumed is null or consumed <= 0 then continue; end if;
    select * into stock_row from public.inventory_stocks where tenant_id=target_tenant_id and product_id=product_id and warehouse_id=reservation_row.warehouse_id for update;
    if not found or stock_row.reserved_quantity < consumed then raise exception 'RESERVATION_STOCK_INCONSISTENT'; end if;
    update public.inventory_stocks set reserved_quantity=reserved_quantity-consumed, updated_at=now() where id=stock_row.id;
  end loop;
  update public.inventory_reservations set status='consumed',released_by=target_user_id,released_at=now(),updated_at=now() where id=target_reservation_id and tenant_id=target_tenant_id;
