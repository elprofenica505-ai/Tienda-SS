-- Fix PL/pgSQL variable/column ambiguity in the cash count path.
begin;
...
begin
  if target_action = 'open' then
    ...
    exception when unique_violation then raise exception 'SESSION_ALREADY_OPEN'; end;
  ...
  for movement_row in select cm.amount as movement_amount, cm.metadata as movement_metadata from public.cash_movements as cm where cm.tenant_id = target_tenant_id and cm.cash_session_id = target_session_id loop
    if coalesce(movement_row.movement_metadata->>'paymentMethod', 'cash') = 'cash' then movements_cash := movements_cash + movement_row.movement_amount; end if;
  end loop;
