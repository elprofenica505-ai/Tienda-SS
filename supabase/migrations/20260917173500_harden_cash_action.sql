...
begin
  if target_action = 'open' then
    ...
    begin
      insert into public.cash_sessions(id,tenant_id,branch_id,cash_register_id,opened_by,status,opening_amount,metadata)
      values(gen_random_uuid(),target_tenant_id,target_branch_id,target_register_id,target_user_id,'open',coalesce((opening->>'cash')::numeric,0),jsonb_build_object('openingByMethod',opening,'currency','NIO'))
  ...
  if target_action = 'movement' then
    ...
    method := target_payload->>'paymentMethod'; amount := greatest(coalesce((target_payload->>'amount')::numeric,0),0); direction := target_payload->>'direction'; description := nullif(left(trim(coalesce(target_payload->>'description','')),300),'');
    if method not in ('cash','card','transfer') or amount <= 0 or direction not in ('in','out') or description is null then raise exception 'INVALID_CASH_MOVEMENT'; end if;
    insert into public.cash_movements(tenant_id,cash_session_id,movement_type,amount,reference_type,performed_by,metadata)
    values(target_tenant_id,target_session_id,case when direction='in' then 'deposit' else 'withdrawal' end,case when direction='in' then amount else -amount end,'manual',target_user_id,jsonb_build_object('paymentMethod',method,'direction',direction,'description',description,'notes',left(coalesce(target_payload->>'notes',''),300))) returning id into new_movement_id;
  ...
  for movement_row in select amount, metadata from public.cash_movements where tenant_id=target_tenant_id and cash_session_id=target_session_id loop
    if movement_row.metadata->>'paymentMethod' = 'cash' then movements_cash := movements_cash + movement_row.amount; end if;
  end loop;
  expected := jsonb_build_object('cash',coalesce((opening->>'cash')::numeric,0)+movements_cash,'card',coalesce((opening->>'card')::numeric,0),'transfer',coalesce((opening->>'transfer')::numeric,0));
  ...
  if target_action in ('close','approve') then
    ...
    if not exists (select 1 from public.members where tenant_id=target_tenant_id and profile_id=target_user_id and role in ('owner','admin','gerente','jefe') and status='active') then raise exception 'MANAGER_REQUIRED'; end if;
