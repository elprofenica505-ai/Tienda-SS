...
import type { Producto, Vista, Permisos } from '@/components/shared/types';
...
export default function BodegaHome({ user, productos, setProductos, irA, volver, historial, onCerrar, permisos }: Props) {
  ...
  const guardarProducto = async (e: React.FormEvent) => {
    ...
    if (!permisos.bodegaCrearProductos) {
      alert('El jefe no te ha habilitado crear productos');
      return;
    }
  ...
  const aplicarAjuste = async () => {
    if (!permisos.bodegaAjustarStock) {
      alert('El jefe no te ha habilitado ajustar stock');
      return;
    }
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {permisos.bodegaRegistrarCompras ? (
            ...
            <div style={{ background: '#1f2937', borderRadius: 12, padding: 14, color: '#6b7280', fontSize: 12, textAlign: 'center' }}>
              Compras deshabilitadas por el jefe
            </div>
          )}
        ...
        {permisos.bodegaCrearProductos ? (
          ...
          <div style={{ background: '#111827', border: '1px solid #374151', borderRadius: 16, padding: 14, color: '#9ca3af', fontSize: 13 }}>
            Crear productos está deshabilitado. Solo el jefe puede activarlo.
          </div>
        )}
        ...
        {filtrados.map(p => (
          <div key={p.id} style={{ background: '#111827', border: `1px solid ${p.stock <= p.stockMinimo ? 'rgba(239,68,68,0.4)' : '#1f2937'}`, borderRadius: 14, padding: 12 }}>
            ...
            {permisos.bodegaAjustarStock ? (
              ...
              <p style={{ marginTop: 8, fontSize: 11, color: '#6b7280' }}>Ajuste de stock deshabilitado por el jefe</p>
            )}
          </div>
        ))}
      </div>
