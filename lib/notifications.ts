...
export async function notifyTenant(tenantId: string, type: AlertType, title: string, message: string, metadata: Record<string, unknown> = {}) {
  ...
  if (resend && from && owner?.email) { try { await resend.emails.send({ from, to: [String(owner.email)], subject: title, html: `<div style="font-family:Arial,sans-serif;max-width:560px"><h2>${escapeHtml(title)}</h2><p>${escapeHtml(message)}</p><p><a href="${appUrl()}/workspace/billing">Abrir facturación</a></p></div>` }); } catch (error) { console.error('notification_email_failed', error instanceof Error ? error.message : 'unknown'); } }
  return alertRef.id;
}
